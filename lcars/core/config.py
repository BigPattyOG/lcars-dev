...
def public_env_path(paths: LcarsPaths) -> Path:
    return Path(
        os.environ.get(
            "LCARS_PUBLIC_ENV_PATH", str(paths.install_root / PUBLIC_ENV_FILE)
        )
    ).expanduser()
...
def build_public_install_payload(profile: InstallProfile) -> dict[str, str]:
    return {
        "LCARS_ENVIRONMENT": profile.environment.upper(),
...
def parse_truthy(value: str | None) -> bool | None:
    if value is None:
        return None
    normalized = value.strip().lower()
    return normalized in {"1", "true", "yes", "on", "configured"}


def load_runtime_config(paths: LcarsPaths | None = None) -> RuntimeConfig:
    resolved_paths = paths or LcarsPaths.discover()
    payload = load_env_file(public_env_path(resolved_paths))
    payload.update(load_env_file(resolved_paths.env_path))
    ...
    return RuntimeConfig(
        ...
        token_configured_flag=parse_truthy(payload.get("LCARS_TOKEN_CONFIGURED")),
        raw=payload,
    )
